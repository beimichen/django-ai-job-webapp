...
class CustomDocument(Document):
    ...
    def write_pdf_and_store_s3(self, target=None, zoom=1, attachments=None, env_url=None,
                               data_type=None):
        ...
        if env_url == 'https://www.jobassistant.com':
            if data_type == 'coverletter':
                bucket_name = 'coverletters-jobassistant'
            elif data_type == 'resume':
                bucket_name = 'resumes-jobassistant'
        else:
            if data_type == 'coverletter':
                bucket_name = 'coverletters-jobassistant-staging'
            elif data_type == 'resume':
                bucket_name = 'resumes-jobassistant-staging'

        try:
            file_obj.seek(0)
            s3.Bucket(bucket_name).put_object(Key=target, Body=file_obj)
            file_obj.close()
...
class CustomHTML(HTML):
    ...
    def write_pdf_and_store_s3(self, target=None, stylesheets=None, zoom=1,
                               attachments=None, presentational_hints=False,
                               font_config=None, env_url=None, data_type=None):
        return self.render(
            stylesheets, enable_hinting=False,
            presentational_hints=presentational_hints,
            font_config=font_config).write_pdf_and_store_s3(
            target, zoom, attachments, env_url, data_type)
